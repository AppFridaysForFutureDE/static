/* Code shamelessly stolen from: https://www.mcc-berlin.net/fileadmin/data/clock/carbon_clock-Dateien/CO2Calculator.js*/
...
var totalBudget = 361 * 1e+9; //1.5 Degree scenario; 2 degrees would be 1111 * 1e+9;
...
function myTimer() {
    ...
    if (getBudgetLeft() > 0) {
        ...
        if (6 < dd.length)
            $("#timecountdown #years").text(parseInt(dd[6]));
        else
            $("#timecountdown #years").text('0');
        if (5 < dd.length)
            $("#timecountdown #months").text(parseInt(dd[5]));
        else
            $("#timecountdown #months").text('0');
        if (4 < dd.length)
            $("#timecountdown #days").text(parseInt(dd[4]));
        else
            $("#timecountdown #days").text('0');
        $("#timecountdown #hours").text(parseInt(dd[3]));
        $("#timecountdown #minutes").text(parseInt(dd[2]));
        $("#timecountdown #seconds").text(parseInt(dd[1]));
        $("#timecountdown #milliseconds").text(dd[0]);  // AR: erased "parseInt" in order to          preserve leading zero
    }
    else {
    ...
    if (getBudgetLeft() > 0) {
        ...
        out.join("")
        $("#carbontonnes").text(out.toLocaleString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1'"));
    }
    else {
